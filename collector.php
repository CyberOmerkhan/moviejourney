...
<body>
	<div class="main">
		...
		<div class="site_content">
			...
			<div class="content">
				<h1>Трейлер фильма "Коллекционер"</h1>
				<iframe width="560" height="315" src="https://www.youtube.com/embed/lq9DV7FsPg8" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
				<div class="film-description">
					<p><span class="label">Фильм: </span><span class = "value-info">Коллекционер</span><span class = "label">Год: </span><span class = "value-info">2009</span><span class="label">Режиссер: </span><span class="value-info">Маркус Данстэн</span></p>
				</div>
				<div class="film-info">
					<hr>
					<h1>Описание фильма "Коллекционер"</h1>
					<img src="img/collector.jpg" alt="Взлом жепы" style = "margin-bottom: 15px">
					Аркин — бывший заключённый, который в составе рабочей артели занимается ремонтом дома семьи Чейз. Бывшая жена Аркина задолжала ростовщикам, поэтому, чтобы защитить её и их общую дочь Синди, Аркин планирует вломиться в дом Чейзов и украсть редкий драгоценный камень, чтобы жена могла отдать свой долг. Вечером того же дня, Аркин вламывается в дом, поднимается по лестнице и начинает взламывать сейф. Однако вскоре он обнаруживает, что кто-то его опередил и подготовил множество смертельных ловушек по всему дому. Теперь задача Аркина усложняется в два раза!
				</div>
				<hr style = "margin-top: 30px">
				<div class="movie">
					<video width="100%" autoplay="autoplay" controls>
						<source src="films/Collection.mp4" type="video/mp4">
					</video>
				</div>
				<hr>
				<h1>Отзывы "Коллекционер"</h1>
				<div class="reviews">
					<?php 
					if(isset($_POST['comment-name'])): 
						...
						$mysql -> query('set character_set_server=utf8');
						$mysql ->  query("SET SESSION collation_connection = 'utf8_general_ci';");
						$mysql -> query("INSERT INTO `collector` (`comment_name`, `comment_text`, `date`) VALUES('$comment_name', '$comment_text', '$date')");
						$comments = $mysql -> query("SELECT * FROM `collector`");
